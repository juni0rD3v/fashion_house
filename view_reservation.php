...
include 'config.php';
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT b.*,p.title,u.address,u.number,concat(u.firstname,' ',u.lastname) as name FROM book_list b inner join `packages` p on p.id = b.package_id inner join users u on u.id = b.user_id where b.id = '{$_GET['id']}' ");
...
<p><b>Apparel Name:</b> <?php echo $title ?>  <br><b>SIZE: </b> <?php echo $size ?>
<br><b>Details:</b><span class="truncate"><?php echo strip_tags(stripslashes(html_entity_decode($title))) ?></span></p>
<p><b>Schedule:</b> <?php echo $schedule ?></p>
<p><b>Full Name:</b> <?php echo $name ?><br><b>Address:</b> <?php echo $address ?><br><b>Contact Number:</b> <?php echo $number ?></p>
