	...
	<div class="card-body">
        ...
        <table class="table table-stripped text-center text-dark">
            ...
            <tbody>
                <?php 
                $i=1;
                    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM rent_list b inner join `packages` p on p.id = b.package_id inner join users u on u.id = b.user_id where b.status = 0 order by date(b.date_created) desc ");
