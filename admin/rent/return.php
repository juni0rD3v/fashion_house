...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title">List of Rents</h3>
	...
	<div class="card-body">
        ...
        <table class="table table-stripped text-center text-dark">
            ...
            <tbody>
                <?php 
                $i=1;
                    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM rent_list b inner join `packages` p on p.id = b.package_id inner join users u on u.id = b.user_id where b.status = 4 order by date(b.date_created) desc ");
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo date("Y-m-d",strtotime($row['sched_pick'])) ?></td>
                        <td><?php echo date("Y-m-d",strtotime($row['sched_return'])) ?></td>
...
<script>
	$(document).ready(function(){
        ...
        $('.view_data').click(function(){
            uni_modal("Rent Information","rent/view.php?id="+$(this).attr('data-id'))
        })
		$('.table').dataTable();
	})
