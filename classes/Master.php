...
Class Master extends DBConnection {
	...
	function delete_package(){
		...
		if($del){
			...
			$this->settings->set_flashdata('success',"Apparel successfully deleted.");
		}else{
			$resp['status'] = 'failed';
	...
	function rent_tour(){
		...
		$save = $this->conn->query("INSERT INTO `rent_list` set $data");
		if($save){
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
	...
	function delete_rent(){
		$del = $this->conn->query("DELETE FROM `rent_list` where id='{$_POST['id']}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata("success","Rent Deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
...
switch ($action) {
	...
	break;
	case 'rent_tour':
		echo $Master->rent_tour();
	break;
	case 'update_book_status':
		echo $Master->update_book_status();
	break;
	case 'update_rent_status':
		echo $Master->update_rent_status();
	break;
	case 'register':
